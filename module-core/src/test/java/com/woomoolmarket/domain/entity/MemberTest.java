...
class MemberTest {
  ...
  void initialAuthProvider() {
    assertThat(member.getInitialAuthProvider()).isEqualTo(AuthProvider.LOCAL);
  }

  @Test
  @DisplayName("Authority 변경 성공")
  void assignAuthorityTest() {
    member.changeAuthority(Role.ADMIN);
  ...
  void getAuthorityFailByWrongAuthority() {
    assertThat(member.getRole()).isNotEqualTo(Role.ADMIN);
  }
  ...
  void editNicknameAndProfileTest() {
    ...
    assertThat(member.getInitialAuthProvider()).isEqualTo(AuthProvider.FACEBOOK);
  }
